...
function calcTotalPrice(products) {
	...
	return productPrices.reduce((acc, curr) => {
		return acc + curr;
	});
}
...
function calcTotalPriceByCategory(products, category) {
	let filterProductByCategory = products.filter(product => (product.category === category));
	let productPrices = filterProductByCategory.map(product => product.price);
	return productPrices.reduce((acc, curr) => {
		return acc + curr;
	});
}
